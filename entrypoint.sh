...
# add a cron job
echo "$CRON_SCHEDULE root rm -rf /backups/* && mysqldump -uroot -p"$PASSWORD" --all-databases --single-transaction --force -h "$MYSQL_HOST" | gzip > /backups/dump.sql.tgz" >> /etc/crontab
crontab /etc/crontab
